...
data "ibm_is_subnet" "zone_1_consul_subnet" {
  identifier = var.ibm_zone_1_subnet_id
}

data "ibm_is_subnet" "zone_2_consul_subnet" {
  identifier = var.ibm_zone_2_subnet_id
}

data "ibm_is_subnet" "zone_3_consul_subnet" {
  identifier = var.ibm_zone_3_subnet_id
}

data "ibm_is_vpc" "consul_vpc" {
  name = data.ibm_is_subnet.zone_1_consul_subnet.vpc_name
}
...
resource "ibm_is_instance" "consul_server_01_instance" {
  ...
  primary_network_interface {
    name            = "internal"
    subnet          = data.ibm_is_subnet.zone_1_consul_subnet.id
    security_groups = [local.security_group_id]
  }
  vpc       = data.ibm_is_subnet.zone_1_consul_subnet.vpc
  zone      = data.ibm_is_subnet.zone_1_consul_subnet.zone
...
resource "ibm_is_instance" "consul_server_02_instance" {
  ...
  primary_network_interface {
    name            = "internal"
    subnet          = data.ibm_is_subnet.zone_2_consul_subnet.id
    security_groups = [local.security_group_id]
  }
  vpc       = data.ibm_is_subnet.zone_2_consul_subnet.vpc
  zone      = data.ibm_is_subnet.zone_2_consul_subnet.zone
  keys      = [data.ibm_is_ssh_key.ssh_key.id]
...
data "template_file" "consul_server_03" {
  ...
  vars = {
    ...
    datacenter           = var.consul_datacenter
    server_1_ip_address  = ibm_is_instance.consul_server_01_instance.0.primary_network_interface.0.primary_ipv4_address
    server_2_ip_address  = var.consul_include ? ibm_is_instance.consul_server_02_instance.0.primary_network_interface.0.primary_ipv4_address : ""
  }
}
...
resource "ibm_is_instance" "consul_server_03_instance" {
  ...
  primary_network_interface {
    name            = "internal"
    subnet          = data.ibm_is_subnet.zone_3_consul_subnet.id
    security_groups = [local.security_group_id]
  }
  vpc       = data.ibm_is_subnet.zone_3_consul_subnet.vpc
  zone      = data.ibm_is_subnet.zone_3_consul_subnet.zone
  keys      = [data.ibm_is_ssh_key.ssh_key.id]
